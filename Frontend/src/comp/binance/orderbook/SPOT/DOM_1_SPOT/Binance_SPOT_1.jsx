...
import {
  ...
  Orderbook_Decimals_1,
} from "../../../../../lib/Binance/Global/GlobalVariables";

import "bootstrap/dist/css/bootstrap.css";

export default function Binance_SPOT_1() {
  ...
  const dataws = () => {
    const depth = new WebSocket(
      "ws://localhost:8080/binance_spot/ws/depth/" + binance_symbol_1
    );
    const wss_aggTrade = new WebSocket(
      "ws://localhost:8080/binance_spot/ws/aggtrade/" + binance_symbol_1
    );

    const ticker = new WebSocket(
      "ws://localhost:8080/binance_spot/ws/ticker5s/" + binance_symbol_1
    );
    ...
    depth.addEventListener("message", (event) => {
      ...
      const bidsData = bids.map((bids) => {
        return {
          price: Number(bids[0]).toFixed(Orderbook_Decimals_1),
          quantity: Number(bids[1]).toFixed(Orderbook_Decimals_1),
        };
      });

      const asksData = asks.map((ask) => {
        return {
          price: Number(ask[0]).toFixed(Orderbook_Decimals_1),
          quantity: Number(ask[1]).toFixed(Orderbook_Decimals_1),
        };
      });
      setData({
        bids: bidsData,
        ...
        >
          ...
          <Tab eventKey="options" title="Settings" className="tab_settings">
            <TabContent className="Tab-DOM-1-Settings">
              ...
              <input
                type="number"
                placeholder={DOM_1_BinanceQuantityFilter_low}
                onChange={(e) => {
                  localStorage.setItem(
                    "DOM_1_BinanceQuantityFilter_low",
                    e.target.value
              ...
              <input
                ...
                placeholder={"insert number "}
                onChange={(e) => {
                  localStorage.setItem("Orderbook_Decimals_1", e.target.value);
                }}
                style={{
                  color: "white",
                  backgroundColor: "black",
