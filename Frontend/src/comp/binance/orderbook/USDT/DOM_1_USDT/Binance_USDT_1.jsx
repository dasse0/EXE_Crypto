...
export default function Binance_USDT_1() {
  ...
  const dataws = () => {
    const depth = new WebSocket(
      "ws://localhost:8080/binance/ws/depth/" + binance_symbol_1
    );
    const wss_aggTrade = new WebSocket(
      "ws://localhost:8080/binance/ws/aggtrade/" + binance_symbol_1
    );

    const ticker = new WebSocket(
      "ws://localhost:8080/binance/ws/ticker5s/" + binance_symbol_1
    );
