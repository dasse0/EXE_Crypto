...
import {
  ...
  DOMFONTSIZE,
  binance_symbol_6,
  DOM_6_Binance_Trade_Quantity_Filter,
  DOM_6_BinanceQuantityFilter_low,
  DOM_6_BinanceQuantityFilter_high,
  DOM_6_Binance_Trade_Quantity_Filter_Highlight,
} from "../../../../../lib/Binance/Global/GlobalVariables";

import "bootstrap/dist/css/bootstrap.css";

import BinanceDOMUSDT6_Orders from "./Binance_DOM_6_Orders";
...
export default function Binance_USDT_2() {
  ...
  const dataws = () => {
    const depth = new WebSocket(
      "ws://localhost:8080/binance/ws/depth/" + binance_symbol_6
    );
    const wss_aggTrade = new WebSocket(
      "ws://localhost:8080/binance/ws/aggtrade/" + binance_symbol_6
    );

    const ticker = new WebSocket(
      "ws://localhost:8080/binance/ws/ticker5s/" + binance_symbol_6
    );
    ...
    wss_aggTrade.addEventListener("message", (event) => {
      ...
      JSON.stringify(data);
      const quantity =
        data.q > DOM_6_Binance_Trade_Quantity_Filter ? data.q : null;
    ...
    <>
      ...
      >
        <div>
          <input
            type="text"
            placeholder={binance_symbol_6}
            onChange={(e) => {
              localStorage.setItem("Binance_Symbol_USDT_DOM_6", e.target.value);
            }}
            style={{
        ...
        >
          ...
          <Tab eventKey="options" title="Settings">
            <TabContent className="Tab-DOM-3-Settings">
              ...
              <input
                type="number"
                placeholder={DOM_6_BinanceQuantityFilter_high}
                inputMode="numeric"
                onChange={(e) => {
              ...
              <input
                type="number"
                placeholder={DOM_6_BinanceQuantityFilter_low}
                onChange={(e) => {
                  localStorage.setItem(
                    "DOM_6_BinanceQuantityFilter_low",
                    e.target.value
              ...
              <input
                type="number"
                placeholder={DOM_6_Binance_Trade_Quantity_Filter}
                onChange={(e) => {
                  localStorage.setItem(
                    "DOM_6_Binance_Trade_Quantity_Filter",
                    e.target.value
              ...
              <input
                type="number"
                placeholder={DOM_6_Binance_Trade_Quantity_Filter_Highlight}
                onChange={(e) => {
                  localStorage.setItem(
                    "DOM_6_Binance_Trade_Quantity_Filter_Highlight",
                    e.target.value
